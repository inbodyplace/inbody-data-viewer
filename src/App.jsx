...
async function fetchDatetimes(account, apiKey, userId) {
  const res = await axios.post(
    '/api/v1/InBody/GetDatetimesByID',
    { USER_ID: userId },
    { headers: makeHeaders(account, apiKey) },
  )
  // Response is typically an array of datetime strings "yyyyMMddHHmmss"
  return Array.isArray(res.data) ? res.data : (res.data?.datetimes ?? [])
}

async function fetchMeasurement(account, apiKey, userId, datetimes) {
  const res = await axios.post(
    '/api/v1/InBody/GetInBodyDataByID',
    { USER_ID: userId, DATETIMES: datetimes },
    { headers: makeHeaders(account, apiKey) },
  )
  return res.data
}
...
export default function App() {
  const [account, setAccount]     = useState(localStorage.getItem('ib_account') || '')
  ...
  const chartOptions = {
    ...
    plugins: {
      legend: { position: 'bottom', labels: { color: '#94a3b8', boxWidth: 12, padding: 16, font: { size: 12 } } },
      title:  { display: false },
    },
    scales: {
    ...
    <div style={styles.root}>
      ...
      <header style={styles.header}>
        ...
        <span style={styles.headerSub}>
          {measurements.length > 0 && `${measurements.length} measurements · UserID: ${userId}`}
        </span>
      </header>
      ...
      <div style={styles.body}>
        ...
        <div style={styles.configPanel}>
          ...
          <input style={styles.input} value={account} onChange={e => setAccount(e.target.value)} placeholder="your_account" />
          <label style={styles.label}>API Key</label>
          <input style={styles.input} value={apiKey} onChange={e => setApiKey(e.target.value)} type="password" placeholder="••••••••••••••••" />
          <div style={{ height: 16 }} />
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            />
            <button style={styles.btn} onClick={search} disabled={loading || !account || !apiKey || !userId}>
              {loading ? '…' : 'Search'}
            </button>
          </div>
          ...
          {measurements.length > 0 && (
            <>
              <div style={{ ...styles.sectionTitle, marginTop: 24 }}>Show Metrics</div>
          ...
          {latest && (
            <>
              <div style={{ ...styles.sectionTitle, marginTop: 24 }}>Latest Snapshot</div>
                ...
                {METRICS.map(m => latest[m.key] != null && (
                  <div key={m.key} style={styles.snapshotCard}>
                    ...
                    <div style={{ fontSize: 18, fontWeight: 700, color: m.color }}>
                      {latest[m.key]}<span style={{ fontSize: 11, fontWeight: 400, color: '#64748b' }}>{m.unit ? ` ${m.unit}` : ''}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div style={styles.main}>
          ...
          {!loading && measurements.length === 0 && !error && (
            <div style={styles.empty}>
              ...
              <div>Enter your API credentials and a User ID to view body composition trends.</div>
            </div>
          )}
          ...
          {measurements.length > 0 && (
            <>
              ...
              {/* Table */}
              <div style={styles.card}>
                <div style={styles.cardTitle}>Measurement History ({measurements.length})</div>
                <div style={{ overflowX: 'auto' }}>
                  <table style={styles.table}>
                    <thead>
                      <tr>
                        <th style={styles.th}>Date</th>
                        {METRICS.map(m => <th key={m.key} style={styles.th}>{m.label}{m.unit ? ` (${m.unit})` : ''}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {[...measurements].reverse().map((r, i) => (
                        <tr key={i} style={{ background: i % 2 === 0 ? 'transparent' : '#0f172a' }}>
                          ...
                          {METRICS.map(m => (
                            <td key={m.key} style={{ ...styles.td, color: m.color }}>
...
const styles = {
  ...
  logo: { fontWeight: 700, fontSize: 15, display: 'flex', alignItems: 'center', gap: 8 },
  headerSub: { fontSize: 12, color: '#64748b' },
  body: { display: 'flex', gap: 0, height: 'calc(100vh - 56px)' },
  configPanel: { width: 260, background: '#1e293b', borderRight: '1px solid #2a3347', padding: '20px 16px', overflowY: 'auto', flexShrink: 0 },
  sectionTitle: { fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748b', marginBottom: 10 },
  label: { display: 'block', fontSize: 12, color: '#94a3b8', marginBottom: 4, marginTop: 10 },
  input: { width: '100%', background: '#0f172a', border: '1px solid #2a3347', borderRadius: 6, padding: '7px 10px', color: '#f1f5f9', fontSize: 13, outline: 'none', boxSizing: 'border-box' },
  btn: { background: '#3b82f6', border: 'none', borderRadius: 6, color: '#fff', padding: '7px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' },
  snapshotGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 },
  snapshotCard: { background: '#0f172a', border: '1px solid #2a3347', borderRadius: 8, padding: '10px' },
  ...
  cardTitle: { fontSize: 13, fontWeight: 700, color: '#f1f5f9', marginBottom: 16 },
  errorBox: { background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '12px 16px', color: '#fca5a5', fontSize: 13 },
  ...
  empty: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#475569', textAlign: 'center', lineHeight: 1.7 },
